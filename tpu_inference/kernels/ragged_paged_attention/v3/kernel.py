...
def _ragged_paged_attention_kernel(*args, merged: bool = False, **kwargs):
    distribution_ref = args[3]

    if merged:
        # Merged path: iterate over groups; each group processes several
        # mixed-session sequences together in a single MXU tile.
        # start/end_seq_idx are derived from distribution_ref inside the loop body.
        ...
        def _(group_idx):
            _ragged_paged_attention_kernel_loop(group_idx, *args, merged=True,
                                                **kwargs)
    else:
        ...
        def _(seq_idx):
            _ragged_paged_attention_kernel_loop(seq_idx, *args, merged=False,
                                                **kwargs)


def _ragged_paged_attention_kernel_loop(
    idx,  # seq_idx (non-merged) or group_idx (merged)
    kv_lens_ref,
    page_indices_ref,
    cu_q_lens_ref,
    # TODO(jevinjiang): merge these into one so we can save SMEM.
    distribution_ref,
    # Remaining positional args differ by path:
    #   non-merged: sem_ids, bo_ids, bkv_update_ids, q, kv, kv_cache, o,
    #               updated_kv_cache, bkv_x2, bq_x2, bo_x2, sems, l, m, acc
    #   merged:     merged_group_cu_seqs, sem_ids, bo_ids, bkv_update_ids,
    #               q, kv, kv_cache, o, updated_kv_cache,
    #               bkv_x2, bq, bo, sems, l, m, acc
    *args_rest,
    merged: bool = False,
    use_causal_mask: bool = True,
    ...
    bq_sz=None,   # bq fetch size (non-merged only)
    bkv_sz=None,  # bkv prefetch size
    ...
    if merged:
        # kv_start_per_q_ref / kv_end_per_q_ref are VMEM scratch refs (appended last).
        (merged_group_cu_seqs_ref,
         ...
         sems, l_ref, m_ref, acc_ref,
         kv_start_per_q_ref, kv_end_per_q_ref) = args_rest
        start_seq_idx, end_seq_idx = RpaCase.MIXED.get_range(distribution_ref)
        group_idx = idx
    else:
        (sem_ids_ref, bo_ids_ref, bkv_update_ids_ref,
         q_hbm_ref, kv_hbm_ref, kv_cache_hbm_ref, o_hbm_ref,
         updated_kv_cache_hbm_ref, bkv_x2_ref, bq_x2_ref, bo_x2_ref,
    ...
    def flash_attention_step1_qk_softmax(
        ...
        *,
        ...
        effective_kv_len,
        q_span_start=None,
        kv_bounds_per_q=None,  # merged path: (kv_start_arr, kv_end_arr) per Q token
    ):
        ...
        if get_dtype_packing(q_dtype) != 1 and get_tpu_version() >= 6:
            int_ty = jnp.int16
        processed_q_len_int = jnp.asarray(processed_q_len, dtype=int_ty)
        processed_kv_len_int = jnp.asarray(processed_kv_len, dtype=int_ty)
        effective_kv_len_int = jnp.asarray(effective_kv_len, dtype=int_ty)
        ...
        mask = None
        if kv_bounds_per_q is not None:
            # Merged path: block-diagonal + causal masking via per-Q-element bounds.
            # kv_bounds_per_q are already expanded to [bq_step*nhd, bkv_csz] by
            # the caller (built without gather via scalar broadcasts per Q token).
            kv_start_2d, kv_end_2d = kv_bounds_per_q
            mask = jnp.logical_and(k_span >= kv_start_2d, k_span < kv_end_2d)
            # Zero out V tokens beyond packed total_kv_len (garbage VMEM).
            if not skip_kv_mask:
    ...
    def wait_fetch_bq(seq_or_q_start, bq_idx_or_sz, bq_sem_idx):
        if merged:
            _fetch_bq(seq_or_q_start, bq_idx_or_sz, bq_sem_idx, wait=True)
        else:
            q_len_start = cu_q_lens_ref[seq_or_q_start] + bq_idx_or_sz * bq_sz
            sz = jnp.minimum(bq_sz, cu_q_lens_ref[seq_or_q_start + 1] - q_len_start)
            _fetch_bq(q_len_start, sz, bq_sem_idx, wait=True)

    def _fetch_group_bkv(group_seq_start, group_seq_end, bkv_sem_idx, *, wait):
        """Fetch KV for all seqs in a group into VMEM slot bkv_sem_idx.

        Caller must reset flat_bkv_state_ref[0] (and [1] if wait=True) to 0 first.
        After return: flat_bkv_state_ref[0] = total_kv_len,
                      flat_bkv_state_ref[1] = total_q_len  (wait=True only).
        """
        @pl.loop(group_seq_start, group_seq_end)
        def _(seq_idx):
            kv_vmem_offset = flat_bkv_state_ref[0]
            kv_len_s = kv_lens_ref[seq_idx]
            q_len_s = cu_q_lens_ref[seq_idx + 1] - cu_q_lens_ref[seq_idx]
            if not wait:
                _fetch_bkv(seq_idx, 0, bkv_sem_idx, vmem_offset=kv_vmem_offset)
            else:
                q_local_offset = flat_bkv_state_ref[1]
                kv_q_gap_s = kv_len_s - q_len_s
                offset, update_sz = _fetch_bkv(seq_idx, 0, bkv_sem_idx, wait=True,
    ...
    def process(static_q_len=None):
        ...
        # Bind merged-path kernel variables at process scope.
        ...
        if merged:
            def _merged_run_attention(total_kv_len_val, bkv_sem_idx):
            ...
            # Step 2: Wait for current group Q and KV; KV wait also fills
            # per-Q bounds for block-diagonal + causal masking.
            wait_fetch_bq(_merged_q_global_start, _total_q_len, _bq_sem_idx)
            total_kv_len_val = wait_fetch_group_bkv(
                _group_seq_start, _group_seq_end, bkv_sem_idx)

            # Step 3: Single attention computation for entire group.
                ...
                def compute_with_bkv(bkv_idx):
                    ...
                    def attention_loop(idx):
                        ...
                        for bq_start in range(0, actual_bq_sz, actual_bq_csz):
                            ...
                            lm_size = actual_bq_csz * num_q_heads_per_kv_head
                            for kv_head_idx in range(actual_num_kv_heads):
                                ...
                                # FlashAttn step1/step2 are interleaved across KV
                                # heads so step2 of the previous head overlaps
                                # step1 of the next.
                                cur_p, cur_v, cur_exp_m_diff = flash_attention_step1_qk_softmax(
                                    bq_c, bk_c, bv_c,
                                    l_ref.at[*lm_slice], m_ref.at[*lm_slice],
    ...
    def prologue():
        if merged:
            ...
            start_fetch_group_bkv(group_seq_start, group_seq_end, sem_ids_ref[1])
        else:
    ...
    @pl.when(True if merged else jnp.logical_and(start_seq_idx <= seq_or_group_idx, seq_or_group_idx < end_seq_idx))
    def pipeline():
    ...
    def run_rpa_kernel(
        ...
        *,
        ...
        if merged:
            ...
            max_q_tokens = compute_size // num_q_heads_per_kv_head
            # Per-Q-element KV bounds in VMEM scratch (appended to scratch_shapes).
            ...
            # [mq, cs]: 1 row per Q token; repeat nhd times in attention.
            scratch_shapes.extend([
                pltpu.VMEM((max_q_tokens, compute_size), jnp.int32),
                pltpu.VMEM((max_q_tokens, compute_size), jnp.int32),
            ])
            scope_name = (
        ...
        kernel_fn = functools.partial(_ragged_paged_attention_kernel, **kernel_kwargs)
